 ...
 *
 * 注：
 *  1. 笔者临时起意，扩展了一下，新增了一个中缀式转后缀式的方法，可以直接计算中缀式的输入，
 *  然后将其转换为后缀式，最后得出结果。思路很简单，用好栈就可以了。
 *  2. 在写练习 3.22 时，发现了一个 BUG，如果中缀表达式为 "4+3"，则会导致其转换为错误的后缀表达式，最后导致
 *  其无法计算出正确的结果，不过好在已经在 3.23 中修改过来了，下面的代码对于表达式有空格的情况是正确的。
 */
